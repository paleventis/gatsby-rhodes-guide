...
// JSON path (relative to JS file in /js/)
fetch('../data/places.json')
  ...
  .then(data => {
    ...
    places.forEach(place => {
      ...
      let distText = '';
      if (userLocation) {
        const d = getDistance(userLocation.lat, userLocation.lng, place.lat, place.lng);
        distText = ` (${d.toFixed(1)} km)`;
      }
      info.textContent = '📍 ' + place.address + distText;
  ...
  .catch(err => {
    console.error(err);
    document.body.innerHTML += '<p>Failed to load places.json</p>';
  });